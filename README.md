...
**『검산하는 인공지능 — 역사부터 에이전트까지, 이론편』의 실습 편**

실습서는 두 권으로 나뉜다.

| 책 | 범위 | 이 저장소 |
|---|---|---|
| **실습 1권 — 딥러닝편** | 1\~25장 | `part1` \~ `part5` |
| **실습 2권 — LLM편** | 1\~19장 | `part6` \~ `part8` (파일 26\~44) |
...
> PowerShell 을 쓴다면 실습 1권 1장의 수동 절차를 따르면 됩니다.
...
> `venv/`, `.env`, `outputs/`, 내려받은 데이터셋은 저장소에 포함되지 않는다.
> 실습을 실행하면 자동으로 만들어진다.
...
> 아래 번호는 **저장소의 파일 번호**다. 책의 장 번호와 다를 수 있다.
>
> - **실습 1권**(1~25장) — 파일 번호와 장 번호가 같다
> - **실습 2권**(1\~19장) — 파일 26\~44가 각각 1\~19장에 해당한다
>   (예: `32_lora_qlora` → 실습 2권 7장)
...
| 2 | `02_numpy_basics` | 4장 | 이론편 손계산 5개 검증 |
...
이론편의 "직접 계산해 보기"에서 손으로 구한 값을 코드로 확인합니다.
...
| 증상 | 해결 |
|---|---|
| `python` 명령을 찾을 수 없음 | PATH 미등록 — 실습 1권 1장 참조 |
| `torch.cuda.is_available()` False | CPU 버전 설치됨 — 실습 1권 1장 |
| `ModuleNotFoundError` | `pip install -r requirements.txt` |
| VS Code 에서 import 실패 | 인터프리터를 venv 로 지정 |
| 그래프 한글이 네모 (□□□) | 실습 1권 3장 1절 폰트 설정 |
| 터미널 한글이 뒤섞임 (`源吏꾩삤`) | 실습 1권 1장 1-7절 — PowerShell UTF-8 설정 |
| `CUDA out of memory` | 배치 축소 — 실습 2권 7장 |

자세한 내용은 `setup_check.ipynb` 를 실행하거나 실습 1권 1장을 참조하세요.
